...
const TYPES_BY_BRAND: Record<string, readonly string[]> = {
  ...
  'N/A': ['CREDIT TRADITIONAL', 'DEBIT CLASSIC', 'CREDIT PERSONAL'],
};
...
function generateCatalog(count = 3000): Product[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    list.push({
      ...
      name,
      category: brand,
      description: `${brand} ${card_type} ${level} issued by ${issuer} (${country}). ZIP ${zip_code}.`,
      image: '',
      status: 'active',
      created_at: now,
      updated_at: now,
...
const CARDS_KEY = 'elite_admin_cards';
