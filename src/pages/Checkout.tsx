    ...
    <div className="min-h-screen bg-bg">
      ...
      <main className="mx-auto max-w-lg px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={() => { setMethod('balance'); setCryptoStep(1); setSelected(null); }}
            className={`rounded-lg border px-3 py-3 text-sm font-medium ${
              method === 'balance' ? 'border-accent bg-accent/10 text-accent' : 'border-border bg-surface text-muted'
            }`}
          >
            Account Balance
          ...
          <button
            ...
            className={`rounded-lg border px-3 py-3 text-sm font-medium ${
              method === 'crypto' ? 'border-accent bg-accent/10 text-accent' : 'border-border bg-surface text-muted'
            }`}
          >
            Pay with Crypto
          </button>
        ...
        {method === 'balance' && insufficient && (
          <Card className="border border-warning/40 bg-warning/10 space-y-3">
            <p className="text-sm font-medium">Insufficient balance</p>
            <Button className="w-full" onClick={() => navigate('/deposit')}>Deposit now</Button>
          </Card>
        )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CRYPTO_OPTIONS.map((c) => (
                    <button
                      ...
                      className="rounded-lg border border-border bg-surface px-3 py-3 text-left text-sm hover:border-accent/50 flex items-center gap-2"
                    >
            ...
            {cryptoStep === 2 && selected && (
              <Card className="space-y-4">
                ...
                <div>
                  ...
                  <Button type="button" size="sm" variant="secondary" className="mt-2" onClick={() => copyAddress(wallet)}>
                ...
                <p className="text-xs text-warning leading-relaxed">
                  Send the exact amount above to this address only on {selected.network}.
                </p>
            ...
            {cryptoStep === 3 && selected && (
              <Card className="space-y-4">
                ...
                <form onSubmit={submitCrypto} className="space-y-3">
                  <Input label="TXID / Transaction Hash" value={txid} onChange={(e) => setTxid(e.target.value)} required />
